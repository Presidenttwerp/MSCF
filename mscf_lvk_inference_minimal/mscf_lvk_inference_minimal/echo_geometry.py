...
def light_ring_radius_M(chi: float) -> float:
    ...
    For chi<0, we switch to the counter-rotating branch (simple choice).
    """
    chi = float(chi)
...
def kerr_rstar_M(r_M: float, chi: float) -> float:
    """
    Kerr tortoise-like coordinate primitive r_*(r) in units of M (geometric units, M=1).

    Uses:
      dr*/dr = (r^2 + a^2)/Delta,  Delta=(r-r+)(r-r-),  a=chi*M.

    Returns r_* in units of M. Additive constants cancel in differences.
    """
    r = float(r_M)
    chi = float(chi)
    chi = np.clip(chi, -1.0 + 1e-15, 1.0 - 1e-15)
...
def delta_t_echo_seconds(M_solar: float, chi: float) -> float:
    ...
    where r_b=M is the MSCF inversion barrier (x_max=2) and r_lr is the light ring.
    """
    G = 6.67430e-11
    c = 299792458.0
    M_sun = 1.98847e30
